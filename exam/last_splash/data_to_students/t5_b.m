...
T_R = table(R2, x, y, R1, 'VariableNames', {'R2', 'x', 'y', 'R1'});

% Perform polynomial fitting
degree = 1;  % Choose the degree of the polynomial (1 for linear)
coeff = polyfit([T_R.R2, T_R.x, T_R.y], T_R.R1, degree);

% Extract coefficients
A_R = double(coeff.Coefficients(1));
B_R = double(coeff.Coefficients(2));
C_R = double(coeff.Coefficients(3));
D_R = double(coeff.Coefficients(4));

%%
% 将数据整合成一个表格
T_G = table(G1, G2, x, y, 'VariableNames', {'R1', 'R2', 'x', 'y'});

% 使用多元线性回归模型拟合数据
lm_G = fitlm(T_G, 'R1 ~ R2 + x + y');
...
%%
% 将数据整合成一个表格
T_B = table(B1, B2, x, y, 'VariableNames', {'R1', 'R2', 'x', 'y'});

% 使用多元线性回归模型拟合数据
lm_B = fitlm(T_B, 'R1 ~ R2 + x + y');
